...
class Status:
    def __init__(self, epoch, accuracy, loss):
        self.epoch = epoch
...
def train(epoch):
    # 记录训练步数
    global total_train_step, epochHistory
    total_train_step = 0
    loss_num = 0
    # 遍历训练集
...
def background_loop():
    ...
    for _ in range(1, epoch + 1):
        ...
        epochHistory.append(
            Status(_, accuracy * 100., loss_num).to_dict())
    currentStatus = True
    # 绘制损失曲线
...
@cross_origin(methods=['GET'])
def reset():
    global currentStatus, epochHistory, train_loss_list
    currentStatus = False
    epochHistory = []
    train_loss_list = []
    return jsonify({'msg': 'OK'})
